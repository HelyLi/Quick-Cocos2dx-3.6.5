    ...
    for (int i = 0; i < [transactions count]; ++i)
    {
        ...
        switch (transaction.transactionState)
        {
            ...
            case SKPaymentTransactionStatePurchasing:
                CCLOG("transaction SKPaymentTransactionStatePurchasing");
                break;
        }
    }
}
...
#define USE_VERIFY_DEFAULT 0

#if USE_VERIFY_DEFAULT
...
- (void)verifyTransactionReceipt:(SKPaymentTransaction *)transaction
{
    ...
    if (verifyOnServer)
    {
        CCLOG("[CCStore_obj] verifyTransactionReceipt() url: %s", utf8cstr(receiptVerifyServerUrl_));
        NSData *data = [receiptVerifyServerUrl_ dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
        server_url = [dic objectForKey:@"server_url"];
        NSNumber * user_id = [dic objectForKey:@"user_id"];
        NSNumber * pro_id = [dic objectForKey:@"pro_id"];
        NSNumber * pro_type = [dic objectForKey:@"pro_type"];
        postData = [[NSString alloc]initWithFormat:@"userId=%d&proType=%d&proId=%d&receipt-data=%s",[user_id intValue],[pro_type intValue],[pro_id intValue],buffer];
    ...
    const char* url = NULL;
    if (verifyOnServer){
        url = [server_url UTF8String];
    }else{
        return;
    }
    
    CCStoreReceiptVerifyRequestIOS* handler = CCStoreReceiptVerifyRequestIOS::create(self, transaction, url);
    handler->getRequest()->addRequestHeader("User-Agent: Mj/2.0");
    handler->getRequest()->addRequestHeader("Content-Type: application/x-www-form-urlencoded");
    handler->getRequest()->setPOSTData([postData cStringUsingEncoding:NSUTF8StringEncoding]);
    handler->getRequest()->start();
}
#endif
